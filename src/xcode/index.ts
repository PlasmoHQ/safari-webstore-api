...
import { filterFilesByExtName, findFileByExtName } from "~util/file"
...
export class XcodeWorkspace {
  ...
  static async findWorkspace(dir: string): Promise<XcodeWorkspace | null> {
    const relativeWorkspacePath = await findFileByExtName(
      dir,
      ".xcworkspace",
      1
    )
    return relativeWorkspacePath
      ? new XcodeWorkspace(relativeWorkspacePath, dir)
      : null
  }
  ...
  static async generate(dir: string, name: string, xcodeprojs: XcodeProject[]) {
    const fileRefs = xcodeprojs.map((xcodeproj) => {
      ...
      return {
        FileRef: [{ _attr: { location: `container:${relativeFilePath}` } }]
      }
    })
    const workspace = {
      Workspace: [{ _attr: { version: "1.0" } }, ...fileRefs]
    }
    ...
    await fs.writeFile(
      `${workspaceDirectory}/contents.xcworkspacedata`,
      xmlString
    )
  }
...
export class XcodeProject extends XcodeWorkspace {
  ...
  static async findProjects(dir: string): Promise<XcodeProject[]> {
    ...
    return workspacePaths.map(
      (relativeFilePath) => new XcodeProject(relativeFilePath, dir)
    )
  }
